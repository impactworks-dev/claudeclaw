...
export async function buildMemoryContext(
  ...
  // Honor the upstream opts. `strictAgentId` (when set) scopes memory lookups
  // to that agent only; when undefined we fall back to `agentId` to preserve
  // our pre-port behavior. includeTeamActivity / includeRecallHistory are
  // accepted for forward-compat — they don't gate anything in our code yet.
  const {
  ...
  void includeTeamActivity; void includeRecallHistory; // forward-compat stubs
  const scopedAgentId = strictAgentId ?? agentId;
  ...
  const searched = searchMemories(chatId, userMessage, 5, queryEmbedding, scopedAgentId);
  ...
  const recent = getRecentHighImportanceMemories(chatId, 5, scopedAgentId);
