...
import { ArrowLeft, ArrowRight, Check, CircleHelp, ContactRound, Mail, Phone, RefreshCw } from 'lucide-preact';
...
interface ReviewData {
  accounts: Account[];
  draft: { selections: Record<string, string>; updatedAt: number };
}
...
export function PrimaryContacts() {
  ...
  const choose = async (contactId: string) => {
    if (!current || saving) return;
    setSaving(true);
    setError('');
    try {
      await apiPut(`/api/primary-contacts/${current.clickupTaskId}`, { contactId });
      setData(prev => prev ? {
        ...prev,
        draft: { selections: { ...prev.draft.selections, [current.clickupTaskId]: contactId }, updatedAt: Date.now() },
      } : prev);
